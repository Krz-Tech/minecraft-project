...
## 6. 構文検証と外部リソースの活用 (Syntax Verification)

AIエージェントは、自身の知識だけに頼らず、以下のプロセスを経てコードを記述すること。

1.  **公式ドキュメントの検索**: 新しい機能の実装や、エラーが再発している箇所の修正を行う際は、必ず `search_web` 等を用いて **[Skript Docs](https://docs.skriptlang.org/)**, **[skUnity](https://docs.skunity.com/)**, または各アドオンの GitHub Wiki を参照すること。
2.  **裏取りの明示**: 思考プロセスにおいて、参照したドキュメントのソース（URL等）を明記し、現在のプロジェクト環境（1.21.4, アドオン構成）に合致しているかを確認した上で実装に移ること。
3.  **既知のエラーパターンの回避**: 同様のエラーが複数回発生している場合、安易な修正（試行錯誤）を繰り返さず、根本的な構文仕様を再調査すること。

## 7. 開発ワークフロー (Development Workflow)

作業の確実性と即時性を担保するため、エージェントは以下の手順を厳守すること。

1.  **コンソール・アタッチ**: 作業（コーディング・修正）を行なう際は、常に `./mc attach s` を使用してサーバーコンソールにアタッチした状態を維持すること。
2.  **構文調査**: 実装前に必要に応じて外部ドキュメントを検索し、正しい構文を確認する。
3.  **即時リロードと検証**: ファイルの保存・変更完了時には、必ず当該スクリプトのリロードコマンド (`sk reload <script_name>`) を実行し、エラーが発生していないことを確認すること。
4.  **エラー解決の優先**: エラーが検出された場合は、他の作業を進める前に最優先で修正・解決すること。
