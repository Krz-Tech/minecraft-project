...
*   **ダメージ計算**: 敵を攻撃した際に出るダメージ数値のポップアップ。
*   **ナビゲーション**: 帰還ポイントまでの距離や方向を空間内に表示。

### 技術仕様
*   **個別パケット表示**: プロトコルパケット経由で召喚するため、他のプレイヤーには一切見えず、戦術的な情報の秘匿と画面のクリーンさを両立します。
*   **Billboard効果**: プレイヤーの視点に合わせて自動回転し、常に正面を向くように設定します。
*   **動的更新**: 数値やカラーをリアルタイムに更新（例：スタッガーが溜まると白色から赤色へ変化）。
